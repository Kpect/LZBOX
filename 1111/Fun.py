...
def GetCurrTime():
    ...
    currTime = str("%d:%d:%d"%(nowDateTime.hour,nowDateTime.minute,nowDateTime.second))
    return currTime
def GetCurrDate():
    """获取当前日期"""
    import datetime
    nowDateTime = datetime.datetime.now()
    currDate = str("%d:%d:%d"%(nowDateTime.year,nowDateTime.month,nowDateTime.day))
    return currDate
G_ResourcesFileList={}
G_CutContent=None
...
def HScrollBar_Config(event,scrollBar):
    ...
    scrollBar.place(x = 0,y = parentWidget.winfo_height()-20,width = parentWidget.winfo_width(),height = 20)
def VScrollBar_Config(event,scrollBar):
    parentinfo = event.widget.winfo_parent()
    parentWidget = event.widget._nametowidget(parentinfo)
    scrollBar.place(x = parentWidget.winfo_width()-20,y = 0,width = 20,height = parentWidget.winfo_height())
...
def MessageBox(text="",title="info",type="info",parent=None):
    ...
    if type == "info":
        if parent:
            tkinter.messagebox.showinfo(title,text,parent=parent)
        else:
            tkinter.messagebox.showinfo(title,text)
    elif type == "error":
        if parent:
            tkinter.messagebox.showerror(title,text,parent=parent)
        else:
            tkinter.messagebox.showerror(title,text)
    else:
        if parent:
            tkinter.messagebox.showwarning(title,text,parent=parent)
        else:
